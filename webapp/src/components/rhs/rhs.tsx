...
const ThreadViewer =
  (window as any).Components.ThreadViewer &&
  styled((window as any).Components.ThreadViewer)`
    height: 100%;
  `;
...
export default function RHS() {
  ...
  const selectedPostId = useSelector(
    (state: any) => state["plugins-" + manifest.id].selectedPostId
  );
  const currentUserId = useSelector<GlobalState, string>(
    (state) => state.entities.users.currentUserId
  );
  const currentTeamId = useSelector<GlobalState, string>(
    ...
    return (
      <RhsContainer>
        <RHSPaddingContainer>
          <RHSImage />
          <RHSTitle>
            <FormattedMessage defaultMessage="Copilot is not yet configured for this workspace" />
          </RHSTitle>
          <RHSText>
            <FormattedMessage defaultMessage="A system admin needs to complete the configuration before it can be used." />
          </RHSText>
          <RHSDivider />
          <RHSSubtitle>
            <FormattedMessage defaultMessage="What is Copilot?" />
          </RHSSubtitle>
          <RHSText>
            <FormattedMessage defaultMessage="Copilot is a plugin that enables you to leverage the power of AI to:" />
          </RHSText>
          <RHSText>
            <ul>
              <RHSBullet>
                <FormattedMessage defaultMessage="Get caught up quickly with instant summarization for channels and threads." />
              </RHSBullet>
              <RHSBullet>
                <FormattedMessage defaultMessage="Create meeting summaries in a flash." />
              </RHSBullet>
              <RHSBullet>
                <FormattedMessage defaultMessage="Ask Copilot anything to get quick answers." />
              </RHSBullet>
            </ul>
          </RHSText>
        </RHSPaddingContainer>
  ...
  if (selectedPostId) {
    ...
    if (threads && bots) {
      content = (
        <ThreadsList data-testid="rhs-threads-list">
          {threads.map((p) => (
            <ThreadItem
              key={p.ID}
              postTitle={p.Title}
              ...
              label={
                bots.find((bot) => bot.dmChannelID === p.ChannelID)
                  ?.displayName ?? ""
              }
              onClick={() => {
                setCurrentTab("thread");

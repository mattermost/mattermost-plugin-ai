...
func (p *AgentsService) MigrateServicesToBots() error {
	...
	p.pluginAPI.Log.Debug("Migrating services to bots")
	...
	err = p.pluginAPI.Configuration.LoadPluginConfiguration(&oldConfig)
	if err != nil {
...
func (p *AgentsService) EnsureBots() error {
	...
	for _, bot := range previousMMBots {
		if _, ok := aiBotConfigsByUsername[bot.Username]; !ok {
			if _, err := p.pluginAPI.Bot.UpdateActive(bot.UserId, false); err != nil {
				p.pluginAPI.Log.Error("Failed to delete bot", "bot_name", bot.Username, "error", err.Error())
				continue
			}
		}
	}
	...
	for _, bot := range cfgBots {
		...
		if prevBot, ok := prevousMMBotsByUsername[bot.Name]; ok {
			if _, err := p.pluginAPI.Bot.Patch(prevBot.UserId, &model.BotPatch{
				...
				p.pluginAPI.Log.Error("Failed to patch bot", "bot_name", bot.Name, "error", err.Error())
				continue
			}
			if _, err := p.pluginAPI.Bot.UpdateActive(prevBot.UserId, true); err != nil {
				p.pluginAPI.Log.Error("Failed to update bot active", "bot_name", bot.Name, "error", err.Error())
				continue
			}
		} else {
			...
			if err != nil {
				p.pluginAPI.Log.Error("Failed to ensure bot", "bot_name", bot.Name, "error", err.Error())
				continue
			}
		}
	}
...
func (p *AgentsService) UpdateBotsCache() error {
...
func (p *AgentsService) GetBotConfig(botUsername string) (llm.BotConfig, error) {
...
func (p *AgentsService) GetBotByUsername(botUsername string) *Bot {
	p.botsLock.RLock()
...
func (p *AgentsService) GetBotByUsernameOrFirst(botUsername string) *Bot {
	bot := p.GetBotByUsername(botUsername)
	if bot != nil {
		return bot
	}
...
func (p *AgentsService) GetBotByID(botID string) *Bot {
	p.botsLock.RLock()
	defer p.botsLock.RUnlock()
	for _, bot := range p.bots {
...
func (p *AgentsService) GetBotForDMChannel(channel *model.Channel) *Bot {
	p.botsLock.RLock()
	defer p.botsLock.RUnlock()
...
func (p *AgentsService) GetBotMentioned(text string) *Bot {
	p.botsLock.RLock()
	defer p.botsLock.RUnlock()
